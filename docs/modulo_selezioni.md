# Modulo Selezioni / Vendite — TRGB Gestionale

**Ultimo aggiornamento:** 2026-05-08 (rinominato da `design_gestione_vendite.md`)
...
**Roadmap:** sezione `C.5` di `docs/roadmap.md` (modulo Cucina-correlato per Marco — "le selezioni dei piatti")

> **Nome modulo:** Marco lo chiama "Selezioni" (selezioni dei piatti dal menu carta che diventano corrispettivi venduti). Tecnicamente è il modulo "Gestione Vendite" che ha sostituito il vecchio "Corrispettivi". Internamente il backend resta `/admin/finance/*` per non rompere routing pre-esistenti.
...
## 1.1 Cosa eredita dal modulo Corrispettivi
...
## 2.1 Routing Frontend

```
/vendite                      — Menu Selezioni (hub + KPI)
...
## 8.6 Pre-conti (superadmin only, 2026-03-23)
...
## 10.3 Endpoint config chiusure — `closures_config_router.py`
...
## 10.4 UI — `CalendarioChiusure.jsx` (dentro Vendite > Impostazioni)
...
## 11.1 Corrispettivi & Stats — `admin_finance.py`
...
# 14. Roadmap modulo (sintesi — dettaglio in `roadmap.md` §K e §S)
