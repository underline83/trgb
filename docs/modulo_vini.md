...
| 📱 Cantina mobile | `/vini/cantina-mobile` | `CantinaMobile.jsx` | **v1.0 "Cantina da iPhone" (2026-08-03, V.9 fase 1)**: pagina mobile-first per il telefono in cantina. Modi Cerca (ricerca + filtro per categoria: Scaffali/Frigo/Matrice) e Per scaffale (**v1.1, 2026-08-21**: searchbar sul nome locazione — matcha anche le etichette contenute — + chip categoria + accordion, una locazione aperta per volta, auto-aperta se ne resta una sola; **2026-09-10, vini 3.88**: il gruppo Matrice è ordinato per posizione — colonna, poi riga, della prima cella del vino — con le celle `(col,riga)` a vista su ogni riga; gli altri gruppi restano alfabetici), + scheda mobile («Dove si trova» con griglia matrice da `LOCAZIONE_3`). **v1.2 (2026-08-21, V.9 fase 2 — la scheda diventa operativa)**: barra azioni fissa in fondo (🍷 venduta −1 / ➕ carico / ✏️ conta), righe di «Dove si trova» toccabili → sheet azioni sul singolo posto (vendita, scarico, carico, conta), toast **Annulla 8s** che fa `DELETE /vini/magazzino/movimenti/{id}` (stesso pattern di CartaStaff), timeline movimenti aperta e raggruppata per giorno con chi/origine/**giacenza risultante** e annulla sull'ultimo, toggle mescita in scheda. Movimenti scritti con `origine='CANTINA-MOBILE'`. **Scrittura riservata ad admin/superadmin/sommelier** (`isViniManagerRole` in `utils/authHelpers.js`, specchio di `is_vini_manager`); la sala vede in sola lettura ma mantiene il toggle mescita. **Due invarianti di sicurezza dati:** (1) la conta corregge per **delta sul posto** (CARICO/SCARICO con nota `[CONTA]`) e **mai con RETTIFICA**, che è assoluta, globale e non tocca le `QTA_<LOC>` → sfaserebbe `QTA_TOTALE` dalla somma dei posti; (2) la **matrice (loc3) resta read-only da mobile**, perché muoverla senza scegliere le celle sfaserebbe `QTA_LOC3` da `matrice_celle`. Fase 3 (conta d'inventario a tappeto) ancora da fare. Dati da `/vini/v2/bottiglie/?only_positive_stock=true`. |
...
- 🚨 **Vini in carta da riordinare** (ex "senza giacenza"; banner alert collassabile, 6 fasi A-F implementate): pill `+ ordina · N` inline con qta suggerita storico Ngg ÷ K (default 60÷2, configurabile via widget settings `qta_suggerita_*`); badge ritmo vendita (top/medio/poco/mai); pill stato riordino (📝 Da ordinare / 📦 Ordinato / 🗓️ Annata esaurita / ⛔ Non ricomprare — 4 stati dopo rimozione 'O' 2026-05-11); chip filtro tipologia (Tutti/Rossi/Bianchi/Bollicine/Rosati/Altri); toggle raggruppa per distributore (persistito in `localStorage`); bottone `✅ Arrivato` inline.
  - 🆕 **RD.1 (2026-08-08, vini 3.81) — è il primo selettore del riordino.** Chi entra non è più `QTA_TOTALE = 0` ma la **copertura in giorni**: `giacenza × finestra ÷ vendite_finestra < alert_carta_giorni_copertura` (default 21; finestra = `qta_suggerita_giorni_storico`). Un vino senza vendite nella finestra non entra mai — 1 bt di un vino che gira è un alert, 1 bt di un vino fermo no. Condizione generata da `app/services/vini_riordino_service.py::sql_da_riordinare()`, **stessa fonte** usata da `riordini_per_fornitore` e dalla pagina `/vini/ordini` (`fornitori_con_lavoro`, `da_ordinare`). Ogni riga mostra giacenza + copertura (`🍷 2 bt · ~9gg`), barra rossa se esaurito / ambra se in esaurimento, banner rosso solo in presenza di esauriti. **RD.1.1 (vini 3.82):** il widget elenca **solo i vini con `STATO_RIORDINO` nullo** — è la coda dei non decisi. Sia 📝 *Da ordinare* sia 📦 *Ordinato* mettono il vino nella **bozza d'ordine del suo fornitore** (`POST /vini/ordini/riga/` con `preserva_qta=true`) e lo togliono dalla lista; il vino appena deciso resta a schermo nel blocco verde «Sistemati adesso» col chip `📦 in bozza · fornitore · N bt` fino al ricaricamento, e ri-cliccando il flag si annulla anche la riga in bozza. **RD.2 (vini 3.83):** ogni riga porta il contesto annate — `➡️ <annata> in cantina · N bt` (cliccabile, apre la bottiglia più recente della stessa `madre_id`) quando la vendemmia successiva è già in giacenza, `➡️ esiste <annata> (0 bt)` quando c'è in anagrafica ma è a zero, e `📥 comprato ~N mesi fa` (ultimo `CARICO` su qualunque annata della madre). Campi da `vini_riordino_service.arricchisci_annate()`: `annata_successiva`, `altre_annate`, `ultimo_acquisto`. Dettaglio: [modulo_vini_ordini.md](modulo_vini_ordini.md) §RD.1, §2-bis, §2-ter.
