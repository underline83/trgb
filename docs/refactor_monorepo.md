...
| R6 | ✅ FATTO | 2026-04-29 | (TBD) | Cleanup `vini.db` legacy (vuoto, mai usato — audit iniziale errato) + nuovo helper `app/utils/locale_data.py::locale_data_path(filename)` con lookup `locali/<locale>/data/` → fallback `app/data/`. NON applicato ai 9 DB attivi (rimandato a R6.5 per separare il rischio). |
