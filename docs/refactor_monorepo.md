...
| **R6.5 (NEW)** | DA FARE | — | — | Applica `locale_data_path()` a tutti i 9 DB SQLite operativi: sostituisce ~10 costanti `Path("app/data/...")` in `core/database.py`, `models/*_db.py`, alcune migrazioni, alcuni router. Sposta i file fisici in `locali/tregobbi/data/` (push 2 separato). Test rigoroso per ogni modulo. Da fare PRIMA di R8 per non far nascere i nuovi moduli con path hardcoded. |
| R7 | ✅ FATTO | 2026-04-30 | (TBD post-push) | Cleanup finale architettura locale: scaffold `locali/_template/` completo (data/seeds/assets gitkeep) + nuovo `docs/architettura_locale.md` (doc canonico modello locale, helper, workflow onboarding cliente nuovo). Verifiche: grep zero stringhe TRGB residue in core (escluse docstring). Sequenza riallineata: R7 ora chiude prima fase refactor, R6.5 e R8 sono evoluzioni successive in sessioni dedicate. |
