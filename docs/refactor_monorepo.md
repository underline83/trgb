...
- I **10 DB SQLite operativi** (`foodcost.db`, `vini.sqlite3`, `vini_settings.sqlite3`, `vini_magazzino.sqlite3`, `dipendenti.sqlite3`, `clienti.sqlite3`, `notifiche.sqlite3`, `tasks.sqlite3`, `bevande.sqlite3`, `admin_finance.sqlite3`) **NON si toccano in R6** — restano in `app/data/`. La sostituzione del path è demandata a R6.5 (sessione successiva), lo spostamento fisico a R6.5 push 2. NB: il piano originale parlava di 9 DB, ma `vini.sqlite3` è ancora attivo (`vini_model.py` + `dashboard_router.py` + `alert_engine.py`).
...
| R6 | ✅ FATTO | 2026-04-29 | `90e1fe7` | Cleanup `vini.db` legacy (vuoto, mai usato — audit iniziale errato) + nuovo helper `app/utils/locale_data.py::locale_data_path(filename)` con lookup `locali/<locale>/data/` → fallback `app/data/`. NON applicato ai 9 DB attivi (rimandato a R6.5 per separare il rischio). |
| **R6.5 push 1** | ✅ FATTO | 2026-05-02 | _(commit pending)_ | Applicato `locale_data_path()` a **10** DB SQLite (non 9 — `vini.sqlite3` ancora attivo via `vini_model.py`/`dashboard_router.py`/`alert_engine.py`). 52 file toccati: 8 modelli + `core/database.py` + `migration_runner.py` + 12 router + 4 servizi + 22 migrazioni + `auth_service.users.json` + `closures_config_router.config.json`. Zero behavior change su tregobbi (fallback storico trova i DB in `app/data/`). TODO inline su 9 tools/scripts one-shot + 4 cartelle uploads (admin_finance/uploads, ipratico_uploads, documenti_dipendenti PDF buste paga, backups vini) — fuori scope, demandato a Modulo K-bis post-R6.5. Verifica: `compileall` 0 errori, smoke import OK, grep finale 0 match in codice eseguibile (solo docstring + bug noto mig 064). |
| **R6.5 push 2** | DA FARE | — | — | Spostamento fisico file DB da `app/data/` a `locali/tregobbi/data/` sul VPS, sotto `[locale:tregobbi]`. Zero downtime: l'helper trova entrambi i path. Coordinato manualmente da Marco con `mv` sul VPS prima del prossimo push.sh, oppure tramite uno script di migrazione idempotente. |
...
| **R8a** | ✅ FATTO | 2026-05-02 | _(commit pending)_ | Manifesti dichiarativi: 13 `core/moduli/<id>/module.json` (vini, ricette, acquisti, controllo_gestione, banca, dipendenti, prenotazioni, clienti, cassa, menu_carta, cucina, task_manager, statistiche) + 1 `core/moduli/platform/module.json` (servizi sempre attivi). 46 router mappati ai moduli. `locali/tregobbi/moduli_attivi.json` e `locali/trgb/moduli_attivi.json` = `{"moduli": ["*"]}` backward-compat. Template documentato con esempi. Zero impatto runtime, contratto per R8b/R8c. |
| **R8b** | DA FARE | — | — | Backend `app/platform/module_loader.py`: legge `moduli_attivi.json` al boot e monta solo i router dei moduli attivi. Default `"*"` o file mancante → tutti attivi (no behavior change su tregobbi). Endpoint nuovo `GET /system/modules` per esporre la lista al frontend. |
| **R8c** | DA FARE | — | — | Frontend `frontend/src/config/moduleLoader.js` + hook `useActiveModules()`: filtra `modulesMenu.js` in base ai moduli attivi. Filtra Home grid + Header dropdown. Default tregobbi: tutti i 13 visibili. |
