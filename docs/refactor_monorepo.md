...
| **R6.5 push 3** | DA FARE (≥ 2026-05-11) | — | — | Rimozione del fallback runtime in `app/utils/locale_data.py::locale_data_path()` per chiudere R6.5 definitivamente (fail-loud invece di fallback). Test in dev prima del push. Pre-deploy: `push.sh` sanity check verifica che TUTTI i DB siano in `locali/tregobbi/data/`. Post-deploy: sanity check + smoke test app. Vedi `docs/r6_5_push_2_runbook.md` ultima sezione. |
