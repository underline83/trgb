...
**Backend — 57 guardie.** Helper locali `_require_admin()` / `_require_turni_write()` per router, coerenti col pattern già usato da `users_router` e `vini_magazzino_router`.
...
- **`turni_router.py` (12)** — scrittura admin, 12 letture aperte, `/riepilogo-dipendenti` admin (restituisce i telefoni). `/miei-turni` resta self-service.
