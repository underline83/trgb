...
**Ultimo aggiornamento:** 2026-09-20 — **DA PUSHARE: Vendite vini, pulsante ↻ «rigenera tempo di apertura» nella card Calici disponibili (vini 3.90) — `POST /vini/magazzino/{id}/bottiglia-aperta/rigenera` riporta `DATA_APERTURA` ad adesso, solo admin/superadmin, traccia `[CALICI-RESET]` nello storico. Nessuna migrazione, nessun build (Vite). Dopo il push: /vini/vendite → card Calici → ↻ su una bottiglia rossa, deve tornare verde «aperta da <1h»**; 2026-09-18 — **DA PUSHARE: Intermittenti → tab «Riepilogo mese» (dipendenti 2.33) — giornate lavorate per dipendente incrociate col registro invii (lavorato vs comunicato), export CSV per il consulente. `GET /intermittenti/riepilogo/`, solo lettura, nessuna migrazione, nessun build (Vite). Nota: il modulo Intermittenti è andato in produzione davvero il 17/09 (primo invio reale, 8 giornate 18→27/09)**; 2026-09-12 — **DA PUSHARE: scadenza della rata editabile dallo Storico spese fisse (controlloGestione 2.22) — nel modale «Storico addebiti» la colonna Scadenza è un campo data per le rate ancora aperte (prima era sola lettura: per affitti/utenze spostare una rata arretrata era un vicolo cieco). Riusa PUT /uscite/{id}/scadenza, `periodo_riferimento` invariato. Solo frontend, nessuna migrazione, nessun build. Dopo il push: Controllo Gestione → Spese Fisse → Storico su un affitto e cambiare una data**; 2026-09-10 — **PUSHATO (verificato live 10/09 sera: versions.jsx servito = 3.89, `GET /vini/magazzino/607/coerenza-giacenza` → 401 e non 404): invariante Matrice (vini 3.89, mig 176) — i movimenti su loc3 passano solo dalle celle, delete/PATCH/creazione/import non scrivono più QTA_LOC3 a mano, banner «Giacenza da sistemare» + Riallinea in scheda, checker M.F `vini_giacenze_incoerenti`. Nessun build (Vite). Dopo il push: aprire una scheda in matrice → Movimenti → Matrice e verificare la scelta celle**; **DA PUSHARE: tasto 📱 Cantina mobile nelle azioni rapide Home (mig 175) per admin/superadmin/sommelier/sala, dopo Cantina Vini; default di reset e fallback FE allineati**; **PUSHATO (`3b9a83ff`, con il fix link quota zero + mig 174): Cantina mobile, matrice ordinata per posizione (vini 3.88) — nel modo «Per scaffale» il gruppo Matrice ordina per (colonna, riga) della prima cella invece che alfabetico, con le celle a vista su ogni riga. Solo frontend, nessun build (il frontend lo serve Vite): dopo il push aprire Per scaffale → Matrice**; 2026-09-08 (notte) — **🚨 DA PUSHARE SUBITO: fix regressione «link a quota zero» + mig 174. `create_link` escludeva dall'allocazione le uscite `PAGATO_MANUALE` (caso normale: fattura segnata pagata a mano, poi arriva la banca) → nessuna rata da allocare → link scritto con `importo_applicato = 0` → in UI «⚡ Parziale: € 0,00 su € 174,22» e fattura non ricollegabile (409). Visto da Marco su 2 bonifici del 30/06 (Balan 174,22 e «dove il lavare è arte» 1.212,17) poche ore dopo il deploy. Criterio corretto: `banca_movimento_id IS NULL` (= non ancora pagata DALLA BANCA) + le PARZIALI. La mig 174 ripara i link già scritti a zero (quota → NULL) e completa le uscite rimaste senza aggancio. Caso 5c aggiunto allo smoke test**; 2026-09-08 (sera) — **FATTO E APPLICATO IN PRODUZIONE (`0362436d`): bonifica dati riconciliazione — mig 173 girata al boot (65 entrate POS orfane cancellate, `foreign_key_check` di foodcost.db 65→0) + `scripts/rettifica_riconciliazioni_2026_09_08.py` lanciato sul VPS il 08/09 alle 23:15 con esito verde su tutti i controlli (backup: `locali/tregobbi/data/foodcost.db.pre-rettifica-20260908-231508`). Le 3 riconciliazioni sbagliate (Bugan/Coffee Lab, Tris Moka, Amazon) sono corrette sul DB vivo: lo script non va rilanciato, le precondizioni ora lo bloccano da sole. Incluso un fix in `banca_router` (il flag «parziale» confrontava a 0,01 invece che con la tolleranza)**; **PUSHATO (`1e48e6fd`): pagamenti parziali in Riconciliazione (flussiCassa 1.21, mig 172) — collegare una fattura non la marca più PAGATA a prescindere: l'importo si alloca davvero, sotto-copertura → `PARZIALE`, soglia arrotondamento configurabile (default 1 €), `banca_fatture_link.importo_applicato` per i pagamenti in più tranche, `is_riconciliato` calcolato dal backend (il tab «Collegati» e il contatore in fondo non si contraddicono più), fix dello «0» fantasma sotto Scollega. Zero movimenti storici cambiano tab (3 chiusi dalla migrazione). Lanciare `npm run build` prima del push**; 2026-09-03 — **🚨 DA PUSHARE SUBITO: fix guardia header-only su `vini_cantina_tools_router` — il commit `ada50c12` ha portato in produzione una guardia costruita su `get_current_user`, che legge solo l'header `Authorization`. I 7 endpoint che si autenticano con `?token=` in query (stampe PDF inventario aperte con window.open, export xlsx, iframe carta cantina) rispondono 401 A TUTTI, admin compreso. Il fix usa la nuova `richiede_ruoli_con()` di M.G. Riguarda anche `/vini/carta/pdf-staff`**; **DA PUSHARE: omaggi nell'imponibile (corrispettivi 4.9, mig 170) — campo `omaggi` in chiusura turno, corrispettivo fiscale = `preconto − annulli + omaggi`, PDF commercialista con colonna «di cui omaggi». Fixa uno scarto di 7,27 € sul 28/08 vs AdE. Fix collaterale: il PDF non sottraeva gli annulli/resi. Quadratura di cassa invariata, storico non recuperabile (Marco corregge a mano). Lanciare `npm run build` prima del push**; 2026-09-01 — **DA PUSHARE: M.G applicato a 25 router (sistema 5.41) — banca, carta, CG, utenze, fatture, FIC, statistiche, alert, clienti, gift card, prenotazioni, preventivi, selezioni, lista spesa, HACCP, ingredienti, menu carta, iPratico. Endpoint con check di ruolo da 200/836 (24%) a **613/836 (73%)**. NON toccati per decisione: modulo Vini (sala e sommelier scrivono, e' il flusso reale) e `chiusure_turno` (la chiusura di cassa serale la fa la sala). ZonaPanel nasconde crea/modifica/elimina a sala e sommelier. ⚠️ regressione nota: `/vendite/chiusure-old` (pagina legacy non linkata) non e' piu' raggiungibile da sala/sommelier**; **DA PUSHARE: M.G fase 1 (sistema 5.41) — `app/services/permessi.py`, guardie di ruolo riutilizzabili (`richiede_ruoli`/`solo_admin`/`verifica_ruoli`/`ha_ruoli`), superadmin implicito, nomi ruolo validati al boot. Chiusi i 3 endpoint PUBBLICI trovati dall'audit: `/foodcost/ingredienti` e `/foodcost/ingredient/{id}` esponevano il listino costi fornitori SENZA token, `/menu/` legacy ora admin+deprecated (cancellazione da confermare con Marco). ⚠️ `VERSION` era 5.39 e `versions.jsx` 5.40: riallineati entrambi a 5.41**; **DA PUSHARE: permessi modulo Dipendenti (dipendenti 2.32) — 59 guardie di ruolo su `dipendenti.py`, `turni_router.py`, `intermittenti_router.py`, `reparti.py` (prima: qualsiasi ruolo autenticato leggeva buste paga, IBAN e codici fiscali), `sub=` sulle 12 route in App.jsx, tab filtrati in DipendentiNav, Foglio Settimana in sola lettura per i non-admin. Nessuna migrazione. Lanciare `npm run build` prima del push. ⚠️ L'audit collaterale su tutti i 56 router ha trovato 636 endpoint su 836 aperti a qualsiasi ruolo autenticato e 3 endpoint PUBBLICI per errore — v. `docs/audit_permessi_2026-09-01.md`, decisione PO aperta su cosa fixare per primo**; **DA PUSHARE: fix mescita reversibile (vini 3.87) — aprire una bottiglia per i calici non accende più `VENDITA_CALICE` (flag anagrafica permanente): chiudendo la mescita il vino esce davvero dalla sezione «Al calice». Solo frontend, nessuna migrazione. Restano 21 vini da ripulire a mano (lista nella sessione). Lanciare `npm run build` prima del push**; **DA PUSHARE: Cantina mobile v1.2 «muovi la bottiglia» (vini 3.86) — la scheda del vino registra movimenti: barra azioni venduta/carico/conta, righe locazione operative, timeline con giacenza risultante, Annulla 8s, toggle mescita. Scrittura ad admin+sommelier. Nessuna modifica backend. Lanciare `npm run build` prima del push**; **PUSHATO: filtro locazioni nel modo «Per scaffale» della Cantina mobile (vini 3.85, commit `7d437a2b`)**; **DA PUSHARE: Gift Card (clienti 3.1, mig 166) — nuova sezione Clienti → 🎁 Gift Card: emissione a valore/esperienza, verifica al banco per codice, scarico a uso unico, annullo/riattiva, PDF A5 del buono, alert scadenza M.F. Non tocca corrispettivi. ⚠️ storico Excel NON ancora importato — lanciare `npm run build` prima del push**; **DA PUSHARE: RD.6 assorbimento widget «Riordini per fornitore» nella pagina Ordini (vini 3.84, chiude B3) — lanciare `npm run build` prima del push**; **PUSHATO: RD.1 + RD.1.1 + RD.2 — Monitor riordino su copertura in giorni, flag che mettono in bozza, contesto annate (vini 3.83, mig 165, commit `f6f1cfcc` + `d7774b89`)**; **DA PUSHARE: Menu Carta multilingua (it/en/fr/es/de/uk) — mig 163 `menu_translations`, `?lang=` su `/menu-carta/public/today` (retrocompatibile), selettore lingua sulla pagina pubblica, tab Traduzioni nel backoffice; menuCarta 1.2→1.3. Include il fix della sezione 'dolci' che non compariva sul QR. Include il seed testi Tre Gobbi (mig 164): EN/FR/ES/DE/UK, 44/44 piatti, 400 righe. Lanciare `npm run build` prima del push**; **DA PUSHARE: verifica docs TOTALE (Blocco 2) — 13 modulo_*.md verificati endpoint per endpoint vs codice e promossi ad `attuale`; trovati 4 bug REALI nel codice (v. sessione 2026-08-04) + 2 decisioni PO aperte**; **DA PUSHARE: docs→wiki, conversione COMPLETATA — header di stato sulle ultime 25 pagine, lint a zero warning (solo docs/, nessun codice)**; **DA PUSHARE: mattone M.J Pubblicazione web (FTP) — bottone "Pubblica sul sito" su menu pranzo e carta vini, sistema 5.39 (richiede le variabili `FTP_*` in `.env` sul VPS, senza quelle il bottone resta disabilitato)**; **DA PUSHARE: Ordini ai fornitori O3–O6 + flag attivo sui distributori — pagina /vini/ordini, invio WhatsApp, migrazioni 158+159+160 (vini 3.77)**; **DA PUSHARE: Intermittenti UNI (comunicazione chiamate dai turni) + mattone M.D email, migrazione 156 — lanciare `npm run build` prima del push**; **DA PUSHARE: La Lavagna** (widget Bacheca sostituito da briefing di servizio in Home + DashboardSala; lanciare `npm run build` prima del push); **DA PUSHARE: verifica docs Blocco 1 — 6 modulo_*.md corretti vs codice (vini, CG, menu carta+pranzo, vendite)**; **PUSHATO: docs→wiki completo (index, convenzioni, 14 pagine, lint in push.sh, log archiviati — v. sessione 2026-07-24)**; **DA PUSHARE: Vista Sommelier v2.0 (vini 3.72, V.22 chiuso)**; e inoltre (dal 19/7): **migrazione 155 self-heal tasks.sqlite3** (il generatore MEP va in 500 finché non parte), **DA PUSHARE: migrazione 155 self-heal tasks.sqlite3** (il generatore MEP va in 500 finché non parte — scoperta perdita template HACCP di aprile, v. TASKS-1 in problemi.md); inoltre restano DA PUSHARE: script rettifica preconti, Vini 3.71, sotto-categorie bevande 3.70, utenze multi-layout (v. sessioni 17-18/7). ⚠️ Nota alle sessioni parallele: changelog/sessione/versions sono stati sovrascritti una volta oggi — rileggere il file da disco PRIMA di scriverci.
