...
// M2.5.5: helper condivisi (sortRows, SortTh) + componente Merge generico.
// Riusiamo gli helper veri (utils/vini/sortableTable) sotto alias "Local" per
// non toccare i call site esistenti in questo file (DenominazioniTable, MadrePanel,
// CrudListTable). I nomi *Local sono storici → ora puntano allo stesso helper.
import { sortRows as sortRowsLocal, SortTh as SortThLocal } from "../../utils/vini/sortableTable";
import MergeAnagraficaModal from "../../components/vini/MergeAnagraficaModal";
import { MadreEditModal } from "../../components/vini/MadreEditModal";
    ...
    <div className="space-y-3">
      ...
      {merging && canEdit && (
        <MergeAnagraficaModal
          ...
          renderSubtitle={c => {
            const parts = [];
            if (c.codice_eambrosia) parts.push(c.codice_eambrosia);
            if (c.nazione) parts.push(c.nazione);
            if (c.regione) parts.push(c.regione);
            return parts.join(" · ") || "—";
          }}
...
// M2.5.5: MergeDenominazioniModal sostituita da MergeAnagraficaModal generico.
